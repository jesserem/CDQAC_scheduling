...
from cdqac.network.main_model import ActorNet, QRDQNNet, DQNNet
...
from cdqac.configs.cdqac_config import TrainConfig
...
def train(config: TrainConfig):
    ...
    # exit()
    if not config.use_dispatching and not config.use_ga_hof and not config.use_ga_pop and not config.use_random and not config.use_eps_pdr:
        raise ValueError("At least one of use_dispatching, use_ga_hof, use_ga_pop must be True")
    job_lenght_list = []
    opt_list = []
    action_list_list = []
    total_runs = 0
    eps_random_ver = f"eps_random_{config.eps_pdr_version}"
    for instance in train_instances:
        ...
        if eps_random_ver in instance and config.use_eps_pdr:
    ...
    # q_optimizer = torch.optim.Adam(q_net.parameters(), lr=config.q_lr, eps=1e-2/config.batch_size, weight_decay=1e-3)
    ...
    collect_data_new(buffer, job_lenght_list, opt_list, action_list_list,
                                                                    ...
                                                                    cutt_off_episode_prob=config.cutt_off_episode_prob,
                                                                    use_proxy_reward=config.use_proxy_reward,
                                                                    use_sparse_reward=config.use_sparse_reward)
    mean_fea_j, mean_fea_m, std_fea_j, std_fea_m, mean_fea_pairs, std_fea_pairs = buffer.normalize_state()


    # mean_fea_j, mean_fea_m, std_fea_j, std_fea_m, mean_fea_pairs, std_fea_pairs = torch.tensor(0), torch.tensor(0), torch.tensor(1), torch.tensor(1), torch.tensor(0), torch.tensor(1)
    print("Data collected")
    # buffer.reward = (config.reward_scale * buffer.reward) + config.reward_bias
    # for i, (makespans, returns, v1, v2) in enumerate(zip(makespan_list, return_list, v1_list, v2_list)):
    #     train_instances[i]["makespan"] = makespans
    #     train_instances[i]["return"] = returns
    #     train_instances[i]["v1"] = v1
    #     train_instances[i]["v2"] = v2

    # buffer.calc_mc_return(config.gamma)
    # collect_data(buffer, train_instances, data_env_func, debug=False)
    # buffer.reward *= 10
    # buffer.reward = 2 * ((buffer.reward - buffer.reward.min()) / (buffer.reward.max() - buffer.reward.min())) - 1
    # buffer.reward = (buffer.reward - buffer.reward.mean()) / (buffer.reward.std() + 1e-6)
    if config.normalize:
        mean_fj, mean_fm, std_fj, std_fm = buffer.normalize_states()
    else:
        mean_fj, mean_fm, std_fj, std_fm = 0, 0, 1, 1
    if config.n_step > 1:
    ...
    if config.do_epoch:
        ...
        for epoch in range(1, config.train_epochs + 1):
            ...
            for batch in buffer.epoch_generator(config.batch_size, device=config.device):
            ...
            if epoch % config.eval_every_epoch == 0:
                ...
                if eval_reward_det < best_det_offline:
                    ...
                    if save_folder is not None:
                        state_dict = trainer.get_dict_with_critic()
                ...
                if save_folder is not None:
                    ...
                    state_dict["mean_fea_pairs"] = mean_fea_pairs
                    state_dict["std_fea_pairs"] = std_fea_pairs
                    torch.save(state_dict, os.path.join(save_folder, f"model_epoch_{epoch}.pt"))
        ...
        for step in range(config.num_train_step_offline):
            ...
            if (step+1) % config.eval_freq == 0:
                ...
                # if save_folder is not None:
                #     state_dict = trainer.get_dict_with_critic()
                #     torch.save(state_dict, os.path.join(save_folder, f"model_{step}.pt"))
                #     if eval_reward_det < best_det:
                #         best_det = eval_reward_det
                #         torch.save(state_dict, os.path.join(save_folder, f"best_det.pt"))
                #     # if eval_reward < best_samp:
                #     #     best_samp = eval_reward
                #     #     torch.save(state_dict, os.path.join(save_folder, f"best_samp.pt"))
                #     if eval_reward_det < best_det_offline:
                #         best_det_offline = eval_reward_det
                #         torch.save(state_dict, os.path.join(save_folder, f"best_det_offline.pt"))
                #     # if eval_reward < best_samp_offline:
                #     #     best_samp_offline = eval_reward
                #     #     torch.save(state_dict, os.path.join(save_folder, f"best_samp_offline.pt"))
                # # print(
                # #     f"Step: {step}, Eval reward Stochastic: {eval_reward}, Eval reward Deterministic: {eval_reward_det}")
                print(
