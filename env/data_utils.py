...
from env.common_utils import strToSuffix
...
def create_jsp_instance(n_j, n_m, low, high):
    """
    :param n_j: number of jobs
    :param n_m: number of machines
    :param low: lower bound of processing time
    :param high: upper bound of processing time
    :return: a jsp instance with shape [N, M]
    """
    times, machines = uni_instance_gen(n_j, n_m, low, high)
    n_op = int(n_j * n_m)
    job_length = np.full(shape=(n_j,), fill_value=n_m, dtype=int)
    op_pt = np.zeros((n_op, n_m), dtype=int)
    times_flat = times.flatten()
    machines_flat = machines.flatten()
    for i in range(n_op):
        op_pt[i][machines_flat[i] - 1] = times_flat[i]
    return job_length, op_pt


def SD2_instance_generator_no_config(n_j: int, n_m: int, low: int, high: int, op_per_mch_min: int, op_per_mch_max: int,
                                     op_per_job: int=0):
    """
