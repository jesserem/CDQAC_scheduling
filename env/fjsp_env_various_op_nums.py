...
import copy
import sys
from env.fjsp_env_same_op_nums import EnvState
import torch
...
class FJSPEnvForVariousOpNums:
    ...
    def __init__(self, n_j, n_m, device='cpu', mask_actions=True):
        self.number_of_jobs = n_j
        self.number_of_machines = n_m
        self.old_state = EnvState()
        self.old_state.device = torch.device(device)
    ...
    def set_initial_data(self, job_length_list, op_pt_list):
        ...
        self.remaining_ops = np.copy(self.job_length)
        ...
        for k in range(self.number_of_envs):
            self.job_remain_work.append(
                [np.sum(self.op_mean_pt[k][self.job_first_op_id[k][i]:self.job_last_op_id[k][i] + 1])
                 for i in range(self.number_of_jobs)])
        self.job_remain_work = np.array(self.job_remain_work)
        self.own_job_remain_work = np.copy(self.job_remain_work)
        # print(len(self.job_remain_work[0]))
        # print(self.job_remain_work[0])
        # print(self.virtual_job_length[0].sum())
        # exit()

        self.op_match_job_remain_work = np.array([np.repeat(self.job_remain_work[k], repeats=self.virtual_job_length[k])
                                                  for k in range(self.number_of_envs)])
        # print(self.op_match_job_remain_work.shape, self.op_mean_pt.shape, self.op_pt.shape)
        # exit()
        # print()
        self.op_match_job_total_work = np.copy(self.op_match_job_remain_work)
        ...
        # print(self.unmasked_op_pt[0])
        # print(self.unmasked_op_pt[0].shape)
        # print(self.candidate[0])
        # print(self.candidate[0].shape)
        # exit()
    ...
    def step(self, actions):
        ...
        self.construct_pair_features()
        self.remaining_ops[:, chosen_job] -= 1
        ...
        self.own_job_remain_work[self.incomplete_env_idx, chosen_job] -= self.op_mean_pt[
            self.incomplete_env_idx, chosen_op]
