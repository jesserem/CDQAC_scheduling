...
from cdqac.utils import open_path, load_yml, load_data_from_files, eval_model_complex, eval_model_complex_no_gap
import numpy as np
...
class TrainConfig:
    ...
    # When True, always use the SD1 generated eval set (never SD2), regardless of
    # what the model config's train_instance says. Benchmark sets (Brandimarte /
    # Hurink) are still evaluated either way.
    sd1_only: bool = False
...
def eval(eval_instance_path, benchmark_result_path, model_path, name_file, save_results_path, device, use_final=False):
    ...
    name, jobLength, opt = load_data_from_files(eval_instance_path)
    ...
    if use_final:
        merged_df.to_csv(os.path.join(save_results_path, f"{name_file}_final.csv"))
    else:
        merged_df.to_csv(os.path.join(save_results_path, f"{name_file}.csv"))
...
def eval_all(config: TrainConfig):
    ...
    if train_instance[0] == "SD1" or config.sd1_only:
        exps = [

            (os.path.join(config.datapath, f'SD1/{type_run}'), os.path.join(config.datapath, 'gen_data.csv'), 'SD1', False),
            (os.path.join(config.datapath, 'BenchData/Brandimarte'),
             os.path.join(config.datapath, 'BenchData/BenchDataSolution.csv'), 'mk', False),
            (os.path.join(config.datapath, 'BenchData/Hurink_edata'),
             os.path.join(config.datapath, 'BenchData/BenchDataSolution.csv'), 'edata', False),
            (os.path.join(config.datapath, 'BenchData/Hurink_rdata'),
             os.path.join(config.datapath, 'BenchData/BenchDataSolution.csv'), 'rdata', False),
            (os.path.join(config.datapath, 'BenchData/Hurink_vdata'),
             os.path.join(config.datapath, 'BenchData/BenchDataSolution.csv'), 'vdata', False),
        ]
    else:
        exps = [
            (
            os.path.join(config.datapath, f'SD2/{type_run}+mix'), os.path.join(config.datapath, 'gen_data.csv'), 'SD2', False),
            (os.path.join(config.datapath, 'BenchData/Brandimarte'),
             os.path.join(config.datapath, 'BenchData/BenchDataSolution.csv'), 'mk', False),
            (os.path.join(config.datapath, 'BenchData/Hurink_edata'),
             os.path.join(config.datapath, 'BenchData/BenchDataSolution.csv'), 'edata', False),
